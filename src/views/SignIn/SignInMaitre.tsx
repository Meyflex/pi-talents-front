...
const validationSchema = Yup.object().shape({
  ...
  const SignInMaitre = observer(() => {
      ...
      const authenticateMaitre =async ()=>{
        try {
          // Envoi avec Axios
          const response = await api.post('/auth/authenticateMaitre', {
            username:signUpMaitreStore.email,
    ...
    <div className='flex justify-center flex-col items-center font-display'>
      ...
      <Formik
        initialValues={{ email: signUpMaitreStore.email, password: signUpMaitreStore.password }}
        validationSchema={validationSchema}
        className='w-full flex flex-col justify-center items-center self-center'
        onSubmit={async (values, actions) => {
            signUpMaitreStore.setEmail(values.email)
            signUpMaitreStore.setPassword(values.password)
            authenticateMaitre()
